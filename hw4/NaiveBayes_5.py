...
class NaiveBayes:
    ...
    def classify(self, words):
        ...
        # Write code here
        pos_score = math.log(self.countdoc['pos']) - math.log(self.totaldoc)
        neg_score = math.log(self.countdoc['neg']) - math.log(self.totaldoc)
        ...
        for word in words:
            pos_score += math.log(self.countpos[word] + smooth)
            pos_score -= math.log(self.totalpos + smooth*len(self.countvoc))
    ...
    def addDocument(self, classifier, words):
        ...
        if self.naiveBayesBool:
            if word not in unilist:
                self.pn[classifier][word] += 1
                unilist.append(word)
            if self.countvoc[word] == 0:
                self.countvoc[word] += 1
            if classifier is 'pos':
                self.totalpos += 1
                self.countpos[word] += 1
            else:
                self.totalneg += 1
                self.countneg[word] += 1
